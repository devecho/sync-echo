...
public class JobResource extends AbstractEndpoint {
	...
	@PUT
	@Path("{jobId}/source")
	public Response updateSource(@PathParam("jobId") String jobId,
			JobSource source) throws ItemAlreadyExistsException {
		JobEntity result = findJob(jobId);
		...
		return Response.ok(sourceConverter.convertTo(result.getSource()))
				.build();
	}

	@PUT
	@Path("{jobId}/target")
	public Response updateTarget(@PathParam("jobId") String jobId,
			JobTarget target) throws ItemAlreadyExistsException {
		JobEntity result = findJob(jobId);
	...
	private void validateJob(Job job) {
		...
		validateFields(job.getData());
	}
	
	private void validateFields(List<Field> fields) {
		for (Field field : fields) {
			if (field.getValue() == null) {
				//TODO use own Exception or ErrorResposne
				throw new BadRequestException(ExceptionMessage
						.format("NULL value is not allowed.")
						.addParameter("field", field.getName()).build());
			}
		}
	}
	...
	private void validateSource(JobSource source) {
		// TODO validate if source or target exists
		validateFields(source.getData());
	}

	private void validateTarget(JobTarget target) {
		// TODO validate if source or target exists
		validateFields(target.getData());
	}
	...
	private JobEntity findJob(String linkId) {
		...
		try {
			...
			throw new WebApplicationException(ErrorResponse.item("jobs")
					.withId(linkId).wasNotFound());
		}
